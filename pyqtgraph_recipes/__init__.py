...
import fourier,mathx
from fourier import FTD
import mathx.uniformly_spaced_vector as usv
...
class ImageWithProjsAndLogAlignedPlot:
    ...
    def __init__(self,gl=None,cornertexts=None):
        ...
        #initialise plots
        self.layout = gl
        self.plots = {'vert':vproj,'horz':hproj,'image':imageplot, 'vert_log':vlog,'horz_log':hlog,'log_image':logimg}
        self.cbar = cbar
...
class PhaseSpacePlot:
    ...
    projections.
    """
    def __init__(self,plots,cbar=None,pen='b',ftd=None,Ex=None,Ek=None,lut=None,S_transform=None,log10_range=3,bound_cond='pad',scale=None):
        """
    ...
    def set_E(self,**kwargs):
        ...
        xs,ks,S=fourier.spectrogram(self.ftd.x.squeeze(),1,self.Ex.squeeze(),32,self.ftd.k.squeeze(),self.bound_cond)
        if self.scale['S']=='max':
            S/=S.max()
        else:
            S/=self.scale['S']
...
class PhaseSpacePlotAligned(ImageWithProjsAlignedPlot,PhaseSpacePlot):
    def __init__(self,gl=None,pen='b',ftd=None,Ex=None,Ek=None,lut=None,
        S_transform=None,cornertexts=None,log10_range=3,bound_cond='pad',scale=None):
        ImageWithProjsAlignedPlot.__init__(self,gl,cornertexts)
        PhaseSpacePlot.__init__(self,self.plots,self.cbar,pen,ftd,Ex,Ek,lut,S_transform,log10_range,bound_cond,scale)

    @classmethod
    def setup_ultrashort_pulse_plot(cls,ftd,log10_range=3,S_transform=None,gl=None,lut=None,t_unit='fs',omega_unit='rad/fs',
                                    It_scale=1,If_scale=1,S_scale=1):
        if gl is None:
            glw=pg.GraphicsLayoutWidget()
            gl=glw.ci
        if lut is None:
            lut=pg.get_colormap_lut()
        scale=dict(x={'fs':1e-15}[t_unit],k={'rad/fs':1e15,'eV':SI['e']/SI['hbar']}[omega_unit],Ix=It_scale,Ik=If_scale,S=S_scale)
        ...
        if S_scale=='max':
            S_str+=' (norm.)'
        psp=PhaseSpacePlotAligned(ftd=ftd,lut=lut,log10_range=log10_range,S_transform=S_transform,scale=scale,gl=gl)
        vp=psp.plots['vert']
...
def plot_radial_grid_lines(plot,rs,color='k',label_format=None,labels=None,label_angle=math.pi/4):
    ...
    def plot_circle(r,label,label_angle):
        ...
        if label is not None:
            text=pg.TextItem(label,color=color,anchor=(0.5,0.8),angle=-90+math.degrees(label_angle))#(0.5,0.8)) # polar_grid_anchor(label_angle)
...
def plot_azimuthal_grid_lines(plot,phis,r=1,color='k',label_format=None,labels=None,unit=math.pi,phi_0=0,phi_dir=1):
    if labels is None:
        if label_format is None:
...
def plot_polar_image(r,phi,data,layout=None,r_label_phi=math.pi/4,
    grid_r=None,levels=None,lut=None,cbar_label=None,phi_0=0,phi_dir=1,
    r_label_fmt='%g',aspect_locked=True,mask_array=None,theta_repeats=None,
    ...
    if len(grid_r)>0:
        plot_radial_grid_lines(plt,grid_r,grid_color,r_label_fmt,label_angle=phi_0+phi_dir*np.asarray(r_label_phi))
        phi_lines=(np.round(phi[-1]/(math.pi/2))-np.arange(4))*math.pi/2
        plot_azimuthal_grid_lines(plt,phi_lines,grid_r[-1],grid_color,'%g&pi;',unit=math.pi,phi_0=phi_0,phi_dir=phi_dir)
